...
public class ImageWindow extends Frame implements FocusListener, WindowListener, WindowStateListener, MouseWheelListener {
    ...
    public ImageWindow(ImagePlus imp, ImageCanvas ic) {
		...
		if (previousWindow!=null) {
			...
			imp.changes = false;
			previousWindow.closeDontNotifyListeners();
			imp.changes = changes;
			if (unlocked)
				imp.unlock();
    ...
    public String createSubtitle() {
    	...
    	try {
			VideoFrameInfo video=new VideoFrameInfo(imp.getProcessor().getPixels());
			s+=" t: "+video+"; ";
		} catch (MissingMagicNumber e) {
			//Frame does not have time stamp
		}
	...
	/** Removes this window from the window list and disposes of it.
	Returns false if the user cancels the "save changes" dialog. */
	public boolean closeDontNotifyListeners() {
	boolean isRunning = running || running2;
	running = running2 = false;
	if (isRunning) IJ.wait(500);
	if (ij==null || IJ.getApplet()!=null || Interpreter.isBatchMode() || IJ.macroRunning())
	...
	if (ij!=null && ij.quitting())  // this may help avoid thread deadlocks
		return true;
	dispose();
	imp.flushDontNotifyListeners();
	
	if (closed && (this instanceof StackWindow)){
		((StackWindow) this).done=true;
		synchronized(this){
			((StackWindow) this).notifyAll();
		}
	}
	...
	public void mouseWheelMoved(MouseWheelEvent event) {
		synchronized(this) {
			...
			if (event.getModifiers()==1 || event.getModifiers()==4){//1 seems to be the shift key (does not work over VNC), 4 command key (for VNC)
				double max=imp.getDisplayRangeMax();
				double min=imp.getDisplayRangeMin();
				double step;
				if (max!=0) step=(max/12.0d)*((double) rotation);
				else step=1.0;
				//IJ.log(""+(max+step));
				if ((max+step>0.01)&&Math.abs(max+step)<1.0e7) imp.setDisplayRange(min,max+step);
				imp.updateChannelAndDraw();
				IJ.showStatus("Set max to "+(max+step));
			} else if (event.getModifiers()==8) {//8 seems to be the option key
				if (imp.isHyperStack()){
					int c=imp.getChannel();
					int z=imp.getSlice();
					int t=imp.getFrame();
					int nChannels=imp.getNChannels();
					imp.setPosition(min(max(1,c+rotation),nChannels),z,t);
				}
				findMax();
			}
		}
