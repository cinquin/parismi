...
public class PolygonRoi extends Roi implements Cloneable {
	...
	*/
	...
	public synchronized Object simpleClone(){
		PolygonRoi r = (PolygonRoi)super.clone();
		r.xp=(int [])xp.clone();
		r.yp=(int [])yp.clone();
		r.xp2=(int [])xp2.clone();
		r.yp2=(int [])yp2.clone();
		r.nPoints=nPoints;
		r.xSpline=(float [])xSpline.clone();
		r.ySpline=(float [])ySpline.clone();
		
		r.splinePoints=splinePoints;
		
	    if (clip!=null) r.clip=(Rectangle) clip.clone();
