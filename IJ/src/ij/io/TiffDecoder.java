...
public class TiffDecoder {
	...
	public void saveImageDescription(byte[] description, FileInfo fi) {
        ...
        if (debugMode) IJ.log("Image description: "+id);
	...
	void dumpTag(int tag, int count, int value, FileInfo fi) {
		...
		if (debugMode) IJ.log("--- Debug info so far "+dInfo+"\n ------------");
	...
	// Get Image File Directory data
		...
		for (int i=0; i<nEntries; i++) {
			...
			if (debugMode) IJ.log(i+"/"+nEntries+" "+tag + ", count=" + count + ", value=" + value);
			...
			switch (tag) {
				...
				case IMAGE_DESCRIPTION: 
					if (ifdCount==1) {
						...
						if (debugMode) IJ.log("Image description is\n"+new String(s));
	...
	void getMetaData(int loc, FileInfo fi) throws IOException {
		...
		if (magicNumber!=MAGIC_NUMBER)  // "IJIJ"
			{in.seek(saveLoc); return;}
		if (debugMode) IJ.log("*****Found the magic number for metadata");
		...
		if (debugMode) IJ.log("*****Done with metadata");
		in.seek(saveLoc);
	}
