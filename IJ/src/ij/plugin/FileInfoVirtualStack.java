...
public class FileInfoVirtualStack extends VirtualStack implements PlugIn {
	...
	/**
	 * Opens a virtual hyperstack into a preexisting ImagePlus
	 * @param targetImp Preexisting (non-null) ImagePlus
	 */
	public void openButDontShow(ImagePlus targetImp){
		FileInfo fi = info[0];
		int n = fi.nImages;
		...
		targetImp.setStack(this);
		//ImagePlus imp2 = new ImagePlus(fi.fileName, this);
		
		targetImp.setFileInfo(fi);
		if (imp!=null && props!=null) {
			targetImp.setCalibration(imp.getCalibration());
			if (fi.info!=null)
				targetImp.setProperty("Info", fi.info);
			...
			if (channels*slices*frames==nImages) {
				targetImp.setDimensions(channels, slices, frames);
				if (getBoolean(props, "hyperstack"))
					targetImp.setOpenAsHyperStack(true);
			}
			/*
			if (channels>1 && fi.description!=null) {
				...
				targetImp = new CompositeImage(targetImp, mode);
			}*/
		}
