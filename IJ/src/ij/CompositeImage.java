...
public class CompositeImage extends ImagePlus {
	...
	public CompositeImage(ImagePlus imp, int mode, ImagePlus copyChannelDisplayRanges) {
		originalImp=copyChannelDisplayRanges;
		createComposite(imp,mode);
	}
	
	private void createComposite(ImagePlus imp, int mode){
			...
			for (int i=0; i<channels; ++i) {
				...
				if (displayRanges!=null) {
					...
					lut[i].max = ip.getMax();
						...
						int originalChannel=originalImp.getChannel();
						int originalSlice=originalImp.getCurrentSlice();
						int originalFrame=originalImp.getFrame();
						originalImp.setPosition(i, originalSlice, originalFrame);
						lut[i].min=originalImp.getDisplayRangeMin();
						lut[i].max=originalImp.getDisplayRangeMax();
						originalImp.setPosition(originalChannel, originalSlice, originalFrame);
					}
				}
			}
			originalImp=null;//to prevent memory leak
			displayRanges = null;
		}
	}
	...
	public void setProcessor(int channel, ImageProcessor ip){
		cip[channel-1]=ip;
	}
