function [dets,feat,matched] = detect_multiscale(stack, model, maxn, donms, sym, pos, overlap, zscale)
% [dets,feat] = detect_multiscale(stack, model, maxn, donms, sym, pos, overlap, zscale)
%                                 1      2      3     4      5    6    7        8
% Args 1,2 are required
% Args 3,4,5 default to inf, false, true, respectively
% Args 6,7,8 are used in latent mode only
%
...
for s = 1:length(scales)
  ...
  zrange = [1 size(scaled,3)];
  [dets{s},feat{s}] = detect_singlescale(scaled, model, ...
