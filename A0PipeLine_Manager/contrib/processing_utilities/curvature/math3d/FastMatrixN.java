...
class FastMatrixN {
	...
	private static void invert(double[][] matrix, boolean showStatus) {
		int M = matrix.length;
	...
	private static double[][] times(double[][] m1, double[][] m2) {
		int K = m2.length;
	 ...
	 */
	private static double[][] LU_decomposition(double[][] m) {
	 ...
	 */
	private static double[][] choleskyDecomposition(double[][] m) {
	...
	private static double[] solve_UL(double[][] A, double[] b) {
	 ...
	 */
	private static double[] solve_cholesky(double[][] A, double[] b) {
	 ...
	 * @return b
	 */
	private static double[] apply(double[][] A, double x[]) {
		int m = A.length;
		int n = A[0].length;
	...
	private static void print(double[][] m, PrintStream out, char del) {
		DecimalFormat f = new DecimalFormat("0.00");
		for (double[] element : m) {
			for (double anElement : element)
	...
	private static float round(double d, int scale, RoundingMode mode) {
		BigDecimal bd = BigDecimal.valueOf(d);
		return (bd.setScale(scale, mode)).floatValue();
	}

	private static void print(double[][] m, PrintStream out) {
		print(m, out, '\t');
	}
	...
	public static void main(String[] args) {
		...
		double[][] m = { { 1, 2, 3, 2 }, { -1, 0, 2, -3 }, { -2, 1, 1, 1 }, { 0, -2, 3, 0 } };
