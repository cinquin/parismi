...
public class JacobiFloat {
	...
	private JacobiFloat(float[][] matrix, int maxSweeps) {
		this.matrix = matrix;
	...
	private static boolean isSmallComparedTo(float value, float reference) {
	...
	private static String toString(float[][] float2Array) {
		String result = "{";
	...
	private static float[][] matMult(float[][] m1, float[][] m2) {
		int r = m1.length;
		int c = m2[0].length;
