...
class JacobiDouble {
	...
	private JacobiDouble(double[][] matrix) {
		this(matrix, 50);
	}

	private JacobiDouble(double[][] matrix, int maxSweeps) {
		this.matrix = matrix;
	...
	private static boolean isSmallComparedTo(double value, double reference) {
		...
		for (int sweeps = 0; sweeps < maxSweeps; sweeps++) {
			...
			for (int p = 0; p < matrix.length - 1; p++)
				for (int q = p + 1; q < matrix.length; q++) {
					...
					if (sweeps > 3 && isSmallComparedTo(tmp, eigenvalues[p]) && isSmallComparedTo(tmp, eigenvalues[q]))
						matrix[p][q] = 0;
						...
						if (isSmallComparedTo(tmp, diff))
							...
							t = 1.0f / (Math.abs(theta) + Math.sqrt(1.0f + theta * theta));
							if (theta < 0)
								t = -t;
						}
						double c = 1.0f / Math.sqrt(1 + t * t);
						double s = t * c;
						double tau = s / (1.0f + c);
	...
	private static String toString(double[][] double2Array) {
		String result = "{";
	...
	private static double[][] matMult(double[][] m1, double[][] m2) {
		int r = m1.length;
		int c = m2[0].length;
