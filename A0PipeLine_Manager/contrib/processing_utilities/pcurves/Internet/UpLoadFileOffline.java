...
class UpLoadFileOffline {
	...
	final protected synchronized void Open() {
		try {
			URL url =
					new URL(processing_utilities.pcurves.Utilities.Environment.cgiRootURL + "/openFile.cgi?file="
	...
	final protected synchronized void Append() {
		...
		try {
			for (int i = 0; i < (stringToAppend.length() - 1) / chunkSize + 1; i++) {
				...
				URL url =
						new URL(processing_utilities.pcurves.Utilities.Environment.cgiRootURL + "/appendFile.cgi?file="
								+ filename + "&string=" + Convert(chunk) + "&uniqueNumber=" + uniqueNumber);
				DataInputStream dIn = new DataInputStream(url.openStream());
