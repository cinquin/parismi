...
import processing_utilities.pcurves.LinearAlgebra.GraphAbstract;
import processing_utilities.pcurves.LinearAlgebra.LineObject;
import processing_utilities.pcurves.LinearAlgebra.LineSegmentAbstract;
import processing_utilities.pcurves.LinearAlgebra.LineSegmentObject;
import processing_utilities.pcurves.LinearAlgebra.OnlineSampleStatisticsDegreeOne;
import processing_utilities.pcurves.LinearAlgebra.Sample;
import processing_utilities.pcurves.LinearAlgebra.SampleDD;
import processing_utilities.pcurves.LinearAlgebra.SampleWithOnlineStatistics;
import processing_utilities.pcurves.LinearAlgebra.Vektor;
import processing_utilities.pcurves.LinearAlgebra.VektorDD;
import processing_utilities.pcurves.Optimize.Optimizable;
...
import processing_utilities.pcurves.PrincipalCurve.Vertex.DummyVertex;
import processing_utilities.pcurves.PrincipalCurve.Vertex.EndVertex;
import processing_utilities.pcurves.PrincipalCurve.Vertex.LineVertex;
import processing_utilities.pcurves.PrincipalCurve.Vertex.RegularVertex;
import processing_utilities.pcurves.PrincipalCurve.Vertex.StarOfManyVertex;
import processing_utilities.pcurves.PrincipalCurve.Vertex.YVertex;
...
import processing_utilities.pcurves.PrincipalCurve.Vertex.Elements.HasTwoSymmetricEdges;
import processing_utilities.pcurves.PrincipalCurve.Vertex.Elements.PenaltyCoefficients;
...
public class PrincipalCurveClass extends GraphAbstract implements Optimizable {
	...
	final public double RepartitionVoronoiRegions() {
		...
		/*
		 * principalCurveParameters.diagnosisTextArea.append(processing_utilities.pcurves.Utilities.Misc.FormatString(
		 * objective,20,1)
		 * + processing_utilities.pcurves.Utilities.Misc.FormatString(MSE,20,1)
		 * + processing_utilities.pcurves.Utilities.Misc.FormatString(penalty,20,1)
		 * + "\n");
		 */
		return MSE;
	}
	...
	final public boolean AddOneVertexAsMidpoint(boolean addAnyway) {
		...
		double max =
				GetEdgeAt(0).GetSetWeight()
						+ (GetEdgeAt(0).GetVertex1().GetSetWeight() + GetEdgeAt(0).GetVertex2().GetSetWeight()) / 2;
		int maxIndex = 0;
		double maxLength = GetLineSegmentAt(0).GetLength();
		for (i = 1; i < GetNumOfLineSegments(); i++) {
			s =
					GetEdgeAt(i).GetSetWeight()
							+ (GetEdgeAt(i).GetVertex1().GetSetWeight() + GetEdgeAt(i).GetVertex2().GetSetWeight()) / 2;
			if (s == max) {
	...
	@Override
	final public void DeletePointAt(int oldVertexIndex) {
		...
		processing_utilities.pcurves.Utilities.MyMath.sortInsertion(oldEdgeIndices);
		for (int l = 0; l < newVertexIndices.length; l++)
		...
		if (getSize() == 0)
			throw (new ArithmeticException("Deleting the vertex would delete the whole graph."));
	}
	...
	final public Sample GetProjections() {
		Sample projections = new SampleDD();
		PrincipalCurveSampleVektor samplePoint;
		for (int i = 0; i < sample.GetSize(); i++) {
			samplePoint = GetSamplePointAt(i);
			projections.AddPoint(samplePoint.Project(GetEdgeAt(samplePoint.GetIndexOfNearestSegment())));
		}
		return projections;
	}

	final public Sample GetProjectionsInOrder() {
		SetOfCurves setOfCurves = ConvertToCurves();
		if (setOfCurves.GetNumOfCurves() != 1)
			return null;
		Curve curve = setOfCurves.GetCurveAt(0);
		curve.UnitParametrize();

		double[] vertexParameters = new double[getSize()];
		for (int i = 0; i < getSize(); i++) {
			for (int j = 0; j < curve.getSize(); j++) {
				if (GetPointAt(i).equals(curve.GetPointAt(j))) {
					vertexParameters[i] = curve.GetParameterAt(j);
					break;
				}
			}
		}
		...
		Sample projections = new SampleDD();
		double[] parameters = new double[sample.GetSize()];
		int[] indices = new int[sample.GetSize()];
		for (int i = 0; i < sample.GetSize(); i++) {
			indices[i] = i;
			PrincipalCurveSampleVektor samplePoint = GetSamplePointAt(i);
			Edge edge = GetEdgeAt(samplePoint.GetIndexOfNearestSegment());
			projections.AddPoint(samplePoint.Project(edge));
			int vertexIndex1 = edge.GetVertexIndex1();
			int vertexIndex2 = edge.GetVertexIndex2();
			int index;
			if (vertexParameters[vertexIndex1] < vertexParameters[vertexIndex2])
				index = vertexIndex1;
			else
				index = vertexIndex2;
			parameters[i] = vertexParameters[index] + GetPointAt(index).Dist2(projections.GetPointAt(i));
		}
		processing_utilities.pcurves.Utilities.MyMath.IndexArray(parameters, indices);
		Sample projectionsInOrder = new SampleDD();
		for (int i = 0; i < sample.GetSize(); i++) {
			projectionsInOrder.AddPoint(projections.GetPointAt(indices[i]));
		}
		return projectionsInOrder;
	}

	final public Sample GetProjectionIndices() {
		SetOfCurves setOfCurves = ConvertToCurves();
		if (setOfCurves.GetNumOfCurves() != 1)
			return null;
		Curve curve = setOfCurves.GetCurveAt(0);
		...
		Sample projections = new SampleDD();
		Sample parameters = new SampleDD();
		for (int i = 0; i < sample.GetSize(); i++) {
			PrincipalCurveSampleVektor samplePoint = GetSamplePointAt(i);
			Edge edge = GetEdgeAt(samplePoint.GetIndexOfNearestSegment());
			projections.AddPoint(samplePoint.Project(edge));
			...
			if (vertexParameters[vertexIndex1] < vertexParameters[vertexIndex2])
				...
				index = vertexIndex2;
			double[] coords = { vertexParameters[index] + GetPointAt(index).Dist2(projections.GetPointAt(i)) };
			parameters.AddPoint(new VektorDD(coords));
		}
		return parameters;
	}

	public void initializeForGeodesicDistance() {
		SetOfCurves setOfCurves = ConvertToCurves();
		if (setOfCurves.GetNumOfCurves() != 1)
			throw new RuntimeException("Wrong number of curves");
		Curve curve = setOfCurves.GetCurveAt(0);
		curve.UnitParametrize();

		vertexParameters = new double[getSize()];
		for (int i = 0; i < getSize(); i++) {
			for (int j = 0; j < curve.getSize(); j++) {
				if (GetPointAt(i).equals(curve.GetPointAt(j))) {
					vertexParameters[i] = curve.GetParameterAt(j);
	...
	private double[] vertexParameters;

	public Vektor getGeodesicDistances(Vektor samplePoint) {

		Edge closestEdge = null;
		double minDist = Double.MAX_VALUE;
		for (Edge edge : edges) {
			double dist = samplePoint.Dist2(edge);
			if (dist < minDist) {
				minDist = dist;
				closestEdge = edge;
			}
		}
		Vektor projection = samplePoint.Project(closestEdge);
		int vertexIndex1 = closestEdge.GetVertexIndex1();
		int vertexIndex2 = closestEdge.GetVertexIndex2();
		int index;
		if (vertexParameters[vertexIndex1] < vertexParameters[vertexIndex2])
			index = vertexIndex1;
		else
			index = vertexIndex2;
		double[] coords =
				{ vertexParameters[index] + GetPointAt(index).Dist2(projection), projection.Sub(samplePoint).Norm2() };
		return new VektorDD(coords);
	}

	// ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// interface Paintable BEGIN
	// ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	@Override
	public void PrintToPostScript(DataPostScriptDocument ps, double pixelWidth, String type, String title) {
		if (type.equals("")) {
			SetOfCurves setOfCurves = ConvertToCurves();
			if (setOfCurves != null)
				setOfCurves.PrintToPostScript(ps, pixelWidth, type, title);
			if (principalCurveParameters.paintProjectionPoints) {
				try {
					ps.GSave();
					...
					ps.NewPath();
					PrincipalCurveSampleVektor samplePoint;
					Vektor projectionPoint;
					for (int i = 0; i < sample.GetSize(); i++) {
						samplePoint = GetSamplePointAt(i);
						projectionPoint = samplePoint.Project(GetEdgeAt(samplePoint.GetIndexOfNearestSegment()));
						ps.Push(ps.ConvertXCoordinate(samplePoint.GetCoordX(ps.projectionPlane)));
						ps.Push(ps.ConvertYCoordinate(samplePoint.GetCoordY(ps.projectionPlane)));
						ps.Push(ps.ConvertXCoordinate(projectionPoint.GetCoordX(ps.projectionPlane)));
						ps.Push(ps.ConvertYCoordinate(projectionPoint.GetCoordY(ps.projectionPlane)));
					}
					ps.NewLine();
					ps.SetLineWidth((pixelWidth + 1) * ps.GetWidthUnit() / 5.0);
					ps.BeginFor(1, 1, sample.GetSize() + 1);
					{
						ps.Pop();
						ps.MoveTo();
						ps.LineTo();
			...
			super.PrintToPostScript(ps, pixelWidth, type, title);
	}

	@Override
	final public void Paint(Graphics g, DataCanvas canvas, int pixelSize, String type) {
		if (principalCurveParameters.paintProjectionPoints && type.equals("")) {
			Color oldColor = g.getColor();
			g.setColor(new Color(oldColor.getRed() + 2 * (255 - oldColor.getRed()) / 3, oldColor.getGreen() + 2
					* (255 - oldColor.getGreen()) / 3, oldColor.getBlue() + 2 * (255 - oldColor.getBlue()) / 3));
			PrincipalCurveSampleVektor samplePoint;
			for (int i = 0; i < sample.GetSize(); i++) {
				try {
					samplePoint = GetSamplePointAt(i);
					Point p1 = canvas.Convert(samplePoint);
					Point p2 = canvas.Convert(samplePoint.Project(GetEdgeAt(samplePoint.GetIndexOfNearestSegment())));
	...
	final class PrincipalCurveSample extends SampleWithOnlineStatistics {
		...
		final public LineObject FirstPrincipalComponentSmallSample(int maxpoints, int randomSeed) {
			processing_utilities.pcurves.LinearAlgebra.SampleWithOnlineStatisticsDegreeTwo smallSample =
					new processing_utilities.pcurves.LinearAlgebra.SampleWithOnlineStatisticsDegreeTwo(this.sample
							.RandomSample(maxpoints, randomSeed));
