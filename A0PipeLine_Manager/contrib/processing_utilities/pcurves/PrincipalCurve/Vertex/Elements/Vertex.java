package processing_utilities.pcurves.PrincipalCurve.Vertex.Elements;
...
import processing_utilities.pcurves.LinearAlgebra.Weighted;
import processing_utilities.pcurves.PrincipalCurve.PrincipalCurveSampleVektor;

abstract public class Vertex extends VektorDD {
	...
	final public Vektor NewVertex() {
		...
		if (processing_utilities.pcurves.Utilities.MyMath.equals(nd.denominator, 0))
			return this;
		else
			return nd.Resolve();
	}
	...
	final protected EndVertexOfTwo MaintainEndVertex(Vertex neighbor) {
		...
		} catch (processing_utilities.pcurves.PrincipalCurve.Vertex.TwoOppositeEdgeIndexesException e1) {
			try {
				processing_utilities.pcurves.PrincipalCurve.Vertex.YVertex realNeighbor =
						(processing_utilities.pcurves.PrincipalCurve.Vertex.YVertex) neighbor;
				return new EndVertexOfY(this, realNeighbor.GetEdgeIndex3(), realNeighbor.GetEdgeIndex1(), realNeighbor
						.GetEdgeIndex2());
			} catch (ClassCastException e2) {
				processing_utilities.pcurves.PrincipalCurve.Vertex.TVertex realNeighbor =
						(processing_utilities.pcurves.PrincipalCurve.Vertex.TVertex) neighbor;
				return new EndVertexOfTwo(this, realNeighbor.GetEdgeIndex(this).edgeIndex);
			}
		}
	}
