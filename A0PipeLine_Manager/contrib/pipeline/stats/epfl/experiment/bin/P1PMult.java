...
import pipeline.stats.epfl.classifier.PMultMixtureModelLearning;
import pipeline.stats.epfl.classifier.VectorClassSizeAdapter2;
import pipeline.stats.epfl.experiment.P1PVectorDemultiplexer;
...
public class P1PMult {
	...
	public static void main(String[] args) {
		try {
			...
			if (args.length < 2) {
				System.out.println("Usage : P1PMult dataFileName experimentFileName [validationFileName]");
			...
			VectorManualPeriodReader learningReader;
			{
				...
				tmp[0] = new LabeledSource(readers[0], nClasses);
				tmp[1] = new UnlabeledSource(readers[1], nClasses);
				if (runPCA)
					learningReader =
			...
			if (runPCA)
				testingReader =
						new VectorPCATransform(new VectorNormalize(new VectorReplace(new LabeledSource(readers[2],
								nClasses), missingVal), mean, stdDev), nComp, evectors, eval);
			else
				testingReader =
						new VectorNormalize(new VectorReplace(new LabeledSource(readers[2], nClasses), missingVal),
								mean, stdDev);
			...
			// Biplots the data to be learnt
			{
				...
				biplot(new VectorClassSizeAdapter2(new VectorUnionReader(tmp), nClasses), confidentRange, "firstPlot");
				learningReader.reset();
			}

			// Learning loop
			debug("First model estimation");
			{
				VectorManualPeriodReader reader =
						new VectorManualPeriodReader(new VectorNormalize(new VectorReplace(new LabeledSource(
								readers[0], nClasses), missingVal), mean, stdDev), false);
				classDpx = new P1PVectorDemultiplexer(reader);
				MultMixtureModelLearning mmml =
						new PMultMixtureModelLearning(mmmc, classDpx, minGrowth, minSigma, nLearningSteps, true);
				mmml.run();
			...
			for (int i = 0; i < nLearningCycles - 1; i++) {

				debug("Creating class demultiplexer");
				classDpx = new P1PVectorDemultiplexer(learningReader);
				...
				double flipRate = (double) error[4] / (double) error[0];
				{
			...
			// Final biplot
			{
				...
				biplot(mmmc, new VectorClassSizeAdapter2(new VectorUnionReader(tmp), nClasses), confidentRange, "plot");
			}
	...
	// This vector reader takes a labeled vector and appends
	// the correct class distribution
	// if the vector is from class 0, appends (1.0, 0.0)
	// else append (0.0, 1.0)
	private static class LabeledSource implements VectorReader {
		VectorReader in;
		final int nClasses;

		public LabeledSource(VectorReader in, int nClasses) {
			this.in = in;
		...
		public double[] read() throws IOException {
			...
			int classIndex = (int) v1[v1.length - 1];
	...
	// the class distribution (1/nClasses, ..., 1/nClasses)
	private static class UnlabeledSource implements VectorReader {
		VectorReader in;
		final int nClasses;

		public UnlabeledSource(VectorReader in, int nClasses) {
			this.in = in;
			this.nClasses = nClasses;
		}
	 ...
	 * ! if a none labeled vector changed class (flip)
	 ...
	 * error[4] = the number of vectors which changed class (flip)
	 */
	private static int[] testingError(Classifier c, double confidentRange, VectorReader testingReader)
		...
		VectorClassSizeAdapter2 testingData = new VectorClassSizeAdapter2(snif, c.nClasses());
		double[] result, model;
		int resultIndex, modelIndex;
		int hardError = 0, unknownError = 0, unknown = 0, flip = 0;

		for (int i = 0; i < testingReader.period(); i++) {
			result = c.classify(testingData.read());
			resultIndex = classIndex(result, confidentRange);

			model = testingData.currentClassDist();
			modelIndex = classIndex(model, confidentRange);
			...
			if (modelIndex == UNKNOWN_CLASS_CODE) {
				if (resultIndex == UNKNOWN_CLASS_CODE) {
					...
					// Updates the class distribution in reader
					int nFeatures = testingData.size();
					for (int j = 0; j < result.length; j++) {
						snif.lastVector()[nFeatures + j] = result[j];
					}
				}
			} else {
				if (resultIndex == UNKNOWN_CLASS_CODE) {
		...
		System.out.println("Unknown : " + unknown + " Flip : " + flip + " Unknown error : " + unknownError
	...
	private static void biplot(VectorClassSizeAdapter2 data, double confidentRange, String fileName) {
		try {
			...
			for (int i = 0; i < biplot.period(); i++) {
				...
				int classIndex = classIndex(data.currentClassDist(), confidentRange);
	...
	private static void biplot(MultMixtureModelClassifier mmmc, VectorClassSizeAdapter2 data, double confidentRange,
			String fileName) {
		try {
			int nClasses = data.nClasses();
			VectorWriter[] out = new VectorWriter[nClasses + 1];
