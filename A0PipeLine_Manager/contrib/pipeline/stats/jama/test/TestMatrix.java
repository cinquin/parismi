package pipeline.stats.jama.test;
...
import pipeline.stats.jama.CholeskyDecomposition;
import pipeline.stats.jama.EigenvalueDecomposition;
import pipeline.stats.jama.LUDecomposition;
import pipeline.stats.jama.Matrix;
import pipeline.stats.jama.QRDecomposition;
 ...
 * </PRE>
 * 
 * </BLOCKQUOTE> Detailed output is provided indicating the functionality being tested and whether the functionality is
 * correctly implemented. Exception handling is also tested.
 * <P>
 * The test is designed to run to completion and give a summary of any implementation errors encountered. The final
 * output should be: <BLOCKQUOTE>
 * 
 * <PRE>
 * <CODE>
 ...
 * </PRE>
 * 
 * </BLOCKQUOTE> If the test does not run to completion, this indicates that there is a substantial problem within the
 * implementation that was not anticipated in the test design. The stopping point should give an indication of where the
 * problem exists.
 **/
class TestMatrix {
	public static void main(String argv[]) {
		...
		try {
			...
			errorCount =
					try_failure(errorCount, "getColumnPackedCopy... ", "data not successfully (deep) copied by columns");
		}
		...
		try {
			...
			if (A.minus(R).norm1() < .001) {
				...
				errorCount =
						try_failure(errorCount, "print()/read()...",
								"Matrix read from file does not match Matrix printed to file");
			}
		} catch (java.io.IOException ioe) {
			warningCount =
					try_warning(warningCount, "print()/read()...",
			...
			try {
				...
				if (A.minus(R).norm1() < .001) {
					...
					errorCount =
							try_failure(errorCount, "print()/read() (2nd attempt) ...",
									"Matrix read from file does not match Matrix printed to file");
				}
			} catch (java.io.IOException ioe) {
				warningCount =
		...
		try {
			...
			try {
				...
				errorCount =
						try_failure(errorCount, "writeObject(Matrix)/readObject(Matrix)...",
								"Matrix not serialized correctly");
			}
		} catch (java.io.IOException ioe) {
			warningCount =
					try_warning(warningCount, "writeObject()/readObject()...",
	...
	private static void check(double x, double y) {
		...
		if (Math.abs(x - y) > 10 * eps * Math.max(Math.abs(x), Math.abs(y))) {
			throw new RuntimeException("The difference x-y is too large: x = " + Double.toString(x) + "  y = "
					+ Double.toString(y));
		}
	}
