package pipeline.stats.jama;

import pipeline.stats.jama.util.Maths;
...
public class SingularValueDecomposition implements java.io.Serializable {
	 ...
	 * 
	 * @param Arg
	 *            Rectangular matrix; returns Structure to access U, S and V.
	 */
		...
		while (p > 0) {
			...
			// Deflate negligible s(p).
				...
				case 3: {
					...
					double scale =
							Math.max(Math.max(Math.max(Math.max(Math.abs(s[p - 1]), Math.abs(s[p - 2])), Math
