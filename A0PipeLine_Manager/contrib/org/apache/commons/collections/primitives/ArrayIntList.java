 ...
 * 
 *         Modified 2010 by Olivier Cinquin for faster access to underlying data, and to remove name
 *         conflicts with the Collection interface.
 */
public class ArrayIntList extends RandomAccessIntList implements IntList, Serializable {
	 ...
	 * Optimized for speed.
	 * 
	 * @return Contents as int []. When possible, a copy is NOT made. The contents of the
	 *         returned array should therefore not be modified.
	 */
	...
	/**
	 * 
	 * @return the backing array. WARNING: It may be larger than the data contained in the list.
	 */
	 ...
	 *            the array to initialize the collection with
	 * @param size
	 *            (must not be larger than length of the array).
	 * @throws NullPointerException
	 *             if the array is <code>null</code>
	 */
	public ArrayIntList(int[] array, int size) {
		...
		if (size > array.length)
			throw new IllegalArgumentException("Requested size is larger than array");
		System.arraycopy(array, 0, _data, 0, array.length);
		_size = size;
	}
	...
	/**
	 * 
	 * Optimized version of {@link #addAll(IntCollection)}for collections that are ArrayIntLists.
	 */
	public final boolean addAll(ArrayIntList collection) {
		return addAll(size(), collection);
	}

	/**
	 * 
	 * Optimized version of {@link #addAll(int, IntCollection)}for collections that are ArrayIntLists.
	 */
	final boolean addAll(int index, ArrayIntList collection) {
		...
		System.arraycopy(collection.getBackingArray(), 0, _data, index, collection.size());
	...
	public boolean addAll(int index, IntCollection collection) {
		...
		for (IntIterator it = collection.intIterator(); it.hasNext();) {
			_data[index] = it.next();
			index++;
		}
		_size += collection.size();
	 ...
	 * return (Iterator<Integer>) intIterator();
	 * }
	 * 
	 * @Override
