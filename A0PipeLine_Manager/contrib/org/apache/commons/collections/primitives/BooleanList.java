...
public interface BooleanList extends BooleanCollection {
	 ...
	 * <p/>
	 * If a collection refuses to add the specified element for any reason other than that it already contains the
	 * element, it <i>must</i> throw an exception (rather than simply returning <tt>false</tt>). This preserves the
	 ...
	 * @param collection
	 *            the {@link BooleanCollection ByteCollection} of
	 *            elements to add
	 * @return <code>true</code> iff I changed as a result of this call
	 * 
	 * @throws UnsupportedOperationException
	...
	/**
	 * Returns <code>true</code> iff <i>that</i> is an <code>BooleanList</code> that contains the same elements in the
	 * same order as me.
	 * In other words, returns <code>true</code> iff <i>that</i> is
	 * a <code>BooleanList</code> that has the same {@link #size() size} as me,
	 * and for which the elements returned by its {@link BooleanList#iterator iterator} are equal (<code>==</code>) to
	 ...
	 * @return <code>true</code> iff <i>that</i> is an <code>BooleanList</code> that contains the same elements in the
	 *         same order as me
	 */
	@Override
	 ...
	 *       <p />
	 *       The hash code of an <code>BooleanList</code> is defined to be the result of the following calculation:
	 * 
	 *       <pre>
	 * int hash = 1;
	 ...
	 * would be returned by an initial call to the {@link BooleanListIterator#next next} method. An initial call to the
	 * {@link BooleanListIterator#previous previous} method would return the element
	 ...
	 * <code><i>fromIndex</i> == 0
	 * && <i>toIndex</i> == {@link #size() size()}</code> the returned list is my "improper" sublist, containing all my
	 * elements.
	 * <p/>
