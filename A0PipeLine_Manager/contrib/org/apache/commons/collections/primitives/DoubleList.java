...
public interface DoubleList extends DoubleCollection {
	...
	/**
	 * Returns <code>true</code> iff <i>that</i> is an <code>DoubleList</code> that contains the same elements in the
	 * same order as me.
	 * In other words, returns <code>true</code> iff <i>that</i> is
	 * a <code>DoubleList</code> that has the same {@link #size() size} as me,
	 * and for which the elements returned by its {@link DoubleList#iterator iterator} are equal (<code>==</code>) to
	 ...
	 * @return <code>true</code> iff <i>that</i> is an <code>DoubleList</code> that contains the same elements in the
	 *         same order as me
	 */
	@Override
	 ...
	 * call to the {@link DoubleListIterator#next next} method. An initial call to the
	 ...
	 * <i>toIndex</i>, exclusive. The returned <code>DoubleList</code> is backed by me, so that any changes in
