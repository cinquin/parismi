...
public class Maths {
	...
	public static double round(double value, int dps) {
		...
		if (n)
			value *= -1;
		double r = Math.round(value * Math.pow(10, dps)) / Math.pow(10, dps);
		if (n)
			r *= -1;
		return r;
	}
	...
	public static double roundDown(double value, int dps) {
		...
		if (n)
			value *= -1;
		double r = Math.floor(value * Math.pow(10, dps)) / Math.pow(10, dps);
		if (n)
			r *= -1;
		return r;
	}
	...
	public static double roundUp(double value, int dps) {
		...
		if (n)
			value *= -1;
		double r = Math.ceil(value * Math.pow(10, dps)) / Math.pow(10, dps);
		if (n)
			r *= -1;
		return r;
	}
