...
public class ExprLexer {
	...
	private ExprLexer(BufferedReader reader) {
		this.reader = new TokenReader(reader);
	}

	private ExprLexer(Reader reader) {
	...
	private static boolean isVariablePart(int lastChar) {
	...
	private static String unescapeString(TokenReader r) throws IOException {
		StringBuilder sb = new StringBuilder();
		char c = 0;
		while (c != '\"') {
			c = (char) r.read();
