...
public class GridMap {
		...
		for (int i = x1; i <= x2; i++) {
			for (int j = y1; j <= y2; j++) {
				...
				if (value == null)
					...
					values.put(k, isArray ? getElement((ExprArray) value, i - x1, j - y1) : value);
			}
		}
	}

	private static Expr getElement(ExprArray array, int x, int y) {
		if (x >= array.columns())
			x = array.columns() - 1;
		if (y >= array.rows())
			y = array.rows() - 1;
	...
	private static long toId(long x, int y) {
		return (x << 32) | y;
	}
}
