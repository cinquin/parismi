...
public class KURT extends ForEachNumberFunction {
	...
	private static void kurt(Counter counter, double value) {
		counter.value3 += Math.pow((value - counter.value) / counter.value2, 4);
	}

	private static void stdev(Counter counter, double value) {
	...
	protected Expr evaluate(Counter counter) throws ExprException {
		...
		double kurt =
				(n * (n + 1) / ((n - 1) * (n - 2) * (n - 3))) * counter.value3 - (3 * Math.pow(n - 1, 2))
						/ ((n - 2) * (n - 3));
